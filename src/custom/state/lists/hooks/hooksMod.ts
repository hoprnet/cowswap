...
function _combineMaps(...maps: TokenAddressMap[]): TokenAddressMap {
  console.count('combineMaps')
  return SUPPORTED_CHAIN_IDS.reduce<Mutable<TokenAddressMap>>((memo, chainId) => {
    const [map1, ...rest] = maps
    // map1 takes precedence
    memo[chainId] = Object.assign({}, ...rest.map((m) => m[chainId]), map1[chainId])
    return memo
  }, EMPTY_LIST) as TokenAddressMap
}
...
export function useUnsupportedTokenList(): TokenAddressMap {
  ...
  return useMemo(() => {
    console.log('useUnsupportedTokenList')
    const unsupported = _combineMaps(brokenListMap, localUnsupportedListMap, loadedUnsupportedListMap)
